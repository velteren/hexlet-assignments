...
public final class App {
    ...
    public static Javalin getApp() {
        ...
        app.post("/articles", ctx -> {
            var nameForCatch = ctx.formParam("title");
            var bodyForCatch = ctx.formParam("content");
            try {
                ...
                var body = ctx.formParamAsClass("content", String.class)
                        ...
                        .get();
                var article = new Article(name, body);
                ArticleRepository.save(article);
                ctx.redirect("articles");
            } catch (ValidationException e) {
                ctx.status(422);
                var page = new BuildArticlePage(nameForCatch, bodyForCatch, e.getErrors());
                ctx.render("articles/build.jte", Collections.singletonMap("page", page));
            }
        });
        // END
